...
MultipleBarPlot_G4<-function(data){
  for (i in 1:nrow(data)){
    data.1<-as.data.frame(data[i,])
    colnames(x.4.1)<-c("NIDO","Menores 1 año","Entre 1 y 3 años", "Entre 3 y 6 años")
    name<-as.character(data.1$NIDO)
    library(reshape2)
    dd=melt(data.1, id=c("NIDO"))
    dd<-dd[,-1]
...
MultipleBarPlot_G5<-function(data){
  for (i in 1:nrow(data)){
    data.1<-as.data.frame(data[i,])
    colnames(x.4.1)<-c("NIDO","Afro","Indígenas", "Blanco/Mestizo")
    name<-as.character(data.1$NIDO)
    library(reshape2)
    dd=melt(data.1, id=c("NIDO"))
    dd<-dd[,-1]
...
MultipleBarPlot_G6<-function(data){
  for (i in 1:nrow(data)){
    data.1<-as.data.frame(data[i,])
    colnames(x.4.1)<-c("NIDO","Adolescentes","18-24 años", "<24 años")
    name<-as.character(data.1$NIDO)
    library(reshape2)
    dd=melt(data.1, id=c("NIDO"))
    dd<-dd[,-1]
